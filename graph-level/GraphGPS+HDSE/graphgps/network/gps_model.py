...
class CateFeatureEmbedding(nn.Module):
    def __init__(self, num_uniq_values, embed_dim, dropout=0.0):
        ...
        self.embeddings = nn.Embedding(num_emb, embed_dim)
...
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.layer_structure_embed_cate = CateFeatureEmbedding(
            self.catelist, cfg.gt.dim_hidden, dropout=0.1
        )
        if (
            cfg.dataset.name == "ogbg-molpcba"
            or cfg.dataset.name == "peptides-structural"
            or cfg.dataset.name == "peptides-functional"
        ):
            self.layer_structure_embed_cate = CateFeatureEmbedding(
                self.catelist, 16, dropout=0.1
            )
    ...
    def forward(self, batch):
        ...
        # print(batch)
        dist = to_dense_adj(
            batch.complete_edge_index, batch.batch, edge_attr=batch.complete_edge_dist
        )
        SPD = to_dense_adj(
            batch.complete_edge_index, batch.batch, edge_attr=batch.complete_edge_SPD
        )
