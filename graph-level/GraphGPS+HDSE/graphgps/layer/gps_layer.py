...
def multi_head_attention_forward(
    ...
    #
    attn_output, attn_output_weights, attn_output_weights_ = (
        bias_scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, dist)
    )
    ...
    if need_weights:
        ...
        attn_output_weights_ = attn_output_weights_.view(
            bsz, num_heads, tgt_len, src_len
        )
        return attn_output, (
            attn_output_weights.sum(dim=1) / num_heads,
            attn_output_weights_.sum(dim=1) / num_heads,
        )
    else:
...
def draw_graph_with_attn(graph, outdir, filename, nodecolor=["tag", "attn"], dpi=300):
    ...
    for i, key in enumerate(nodecolor):
        ...
        if "attn" in key:
            ...
            sm = plt.cm.ScalarMappable(
                cmap=cmap[key], norm=plt.Normalize(vmin=0, vmax=vmax[key])
            )
            sm._A = []
...
class GPSLayer(nn.Module):
    ...
    def __init__(
        ...
        if (
            ...
            or dataset == "peptides-structural"
            or dataset == "peptides-functional"
        ):
            self.structure_map_query = nn.Sequential(nn.Linear(16, num_heads))
            # self.structure_map_key = nn.Sequential(
            #     nn.Linear(16, num_heads))
        else:
            self.structure_map_query = nn.Sequential(nn.Linear(dim_h, num_heads))
            self.structure_map_key = nn.Sequential(nn.Linear(dim_h, num_heads))
